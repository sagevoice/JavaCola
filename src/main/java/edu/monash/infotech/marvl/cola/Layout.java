...
public class Layout {
    ...
    /** subscribe a listener to an event. Sub-class and override this method to replace with a more sophisticated eventing mechanism */
    ...
    /**
     * a function that is notified of events like "tick" sub-class and override this method to replace with a more sophisticated eventing
     * mechanism
     */
    ...
    /**
     * a function that kicks off the iteration tick loop it calls tick() repeatedly until tick returns true (is converged). Subclass and
    ...
    public Layout powerGraphGroups(final Consumer<Groups> f) {
        final Groups g = (new PowerGraph<Link>()).getGroups(this._nodes, this._links, this.linkAccessor, this._rootGroup);
    ...
    private class LinkAccessor implements LinkLengthAccessor<Link>, LinkTypeAccessor<Link>, LinkSepAccessor<Link> {
        ...
        @Override
        public void setLength(final Link l, final double value) {
        ...
        @Override
        public double getMinSeparation(final Link l) {
     ...
     *
     * @param idealLength the base length for an edge when its source and start have no other common neighbours (e.g. 40)
     * @param w           a multiplier for the effect of the length adjustment (e.g. 0.7)
     */
     ...
     *
     * @param initialUnconstrainedIterations unconstrained initial layout iterations
     * @param initialUserConstraintIterations
     *                                       initial layout iterations with user-specified constraints
     * @param initialAllConstraintsIterations
     *                                       initial layout iterations with all constraints including non-overlap
     * @param gridSnapIterations             iterations of "grid snap", which pulls nodes towards grid cell centers - grid of size
                        ...
                        final int initialAllConstraintsIterations, final int gridSnapIterations, final boolean keepRunning)
    {
        ...
        if (0 < gridSnapIterations) {
            ...
            this._descent.G = Descent.createSquareMatrix(N, (i, j) -> {
                if (i >= n || j >= n) {
                    return G[i][j];
                }
                return 0;
        ...
        if (null == this._distanceMatrix && this._handleDisconnected) {
            ...
            final List<Graph> graphs = handleDisconnected.separateGraphs(this._nodes, this._links);
    ...
    /**
     * find a visibility graph over the set of nodes.  assumes all nodes have a bounds property (a rectangle) and that no pair of bounds
     * overlaps.
     */
